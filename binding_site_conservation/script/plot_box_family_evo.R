...
suppressMessages({
  ...
  library(dplyr)        # 用于 group_by 和 filter
})
...
# 7. 箱线图作为“水果”添加到叶节点（纵向）
# 先过滤掉只有一个唯一值或方差为 0 的 RBP
filtered_plot_df <- plot_df %>%
  group_by(RBP) %>%
  filter(n_distinct(value) > 1 & var(value, na.rm = TRUE) > 0) %>%  # ✅ 移除了 !any(value == 0)
  ungroup() %>%
  na.omit()

# ✅ 调试输出：检查过滤后的数据
cat("过滤后的 RBP 数量:", length(unique(filtered_plot_df$RBP)), "\n")
print(head(filtered_plot_df))

# ✅ 新增：如果过滤后无数据，退出并提示
if (nrow(filtered_plot_df) == 0) {
  cat("❌ 过滤后无有效数据，请检查数据是否全为 0 或只有一个唯一值\n")
